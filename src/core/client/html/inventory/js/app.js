...
function FormatItemInfo(itemData) {
    if (itemData != null && itemData.info != '') {
        if (itemData.name == 'id_card') {
            ...
            $('.item-info-title').html('<p>' + itemData.label + '</p>');
            $('.item-info-description').html(
                '<p><strong>Worth: </strong><span>$' + itemData.info.worth + '</span></p>'
            );
                    ...
                    $(this)
                        ...
                        .html(
                            itemData.amount -
                                dragAmount +
                                ' (' +
                                ((itemData.weight * (itemData.amount - dragAmount)) / 1000).toFixed(1) +
                                ')'
                        );
                    $('.ui-draggable-dragging')
                        .find('.item-slot-amount p')
...
function updateweights($fromSlot, $toSlot, $fromInv, $toInv, $toAmount) {
    ...
    if (
        $toAmount == 0 &&
        ($fromInv.attr('data-inventory').split('-')[0] == 'itemshop' || $fromInv.attr('data-inventory') == 'crafting')
    ) {
        itemData = $fromInv.find('[data-slot=' + $fromSlot + ']').data('item');
        if ($fromInv.attr('data-inventory').split('-')[0] == 'itemshop') {
            $fromInv
    ...
    $('#player-inv-weight').html(
        '⚖️: ' + (parseInt(totalWeight) / 1000).toFixed(2) + ' / ' + (playerMaxWeight / 1000).toFixed(2)
    );
    if (
        $fromInv.attr('data-inventory').split('-')[0] != 'itemshop' &&
        $toInv.attr('data-inventory').split('-')[0] != 'itemshop' &&
        ...
        $('#other-inv-weight').html(
            '⚖️: ' + (parseInt(totalWeightOther) / 1000).toFixed(2) + ' / ' + (otherMaxWeight / 1000).toFixed(2)
        );
    }
...
$(document).on('click', '.CombineItem', function (e) {
    ...
    if (combineslotData.toData.combinable.anim != null) {
        alt.emit(
            'inventory:combineWithAnim',
            combineslotData.toData.combinable,
            combineslotData.toData.name,
            combineslotData.fromData.name
        );
        // $.post(
        ...
        alt.emit(
            ...
            combineslotData.toData.combinable.reward,
            combineslotData.toData.name,
            combineslotData.fromData.name
        );
        // $.post(
...
function optionSwitch($fromSlot, $toSlot, $fromInv, $toInv, $toAmount, toData, fromData) {
    ...
    alt.emit(
        ...
        $fromInv.attr('data-inventory'),
        $toInv.attr('data-inventory'),
        $fromSlot,
        $toAmount,
        toData.amount
    );
...
(() => {
    ...
    Inventory.Update = function (data) {
        ...
        $('#player-inv-weight').html(
            '⚖️: ' + (totalWeight / 1000).toFixed(2) + ' / ' + (data.maxweight / 1000).toFixed(2)
        );
